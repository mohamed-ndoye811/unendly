...
const {allCards: cards, dailyCardsAdded, dailyStudiesComplete} = storeToRefs(studyStore);
const {cardsPerDay, startDate, studyHour} = storeToRefs(settingsStore);
const newCards = ref([] as Card[]);

const initCardState = reactive({
    question: 'Question',
    answer: 'Answer',
    category: null,
    theme: null,
    ...
    <main id="add-cards">
        <div class="card-deck" v-if="!dailyStudiesComplete">
            <CardComponent v-for="(card, index) in cardsToAdd"
                           v-bind:key="cards.filter(card => card).length + (cardsPerDay as number) - index" mode="edit"
        ...
        <div class="studies-finished" v-else-if="dailyStudiesComplete">
            <h2 v-if="studyHour > parseInt(moment().format('HH'))">You are early!! Come back at {{ studyHour }}h for your daily study session</h2>
